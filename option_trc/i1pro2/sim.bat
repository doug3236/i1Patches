...
rem also create a 250 patch, uncorrelated random patch set with neutral ramp
echo i1patches -T2 21 -e 250 x6.txt opt957.txt
i1patches -T2 21 -e 250 x6.txt opt957.txt >log.txt
echo - >>log.txt
...
echo i1patches -X .2 .05 P_rand250__x6__opt957.txt p4357_M2.icm
i1patches -X .2 .05 P_rand250__x6__opt957.txt p4357_M2.icm >>log.txt
echo - >>log.txt

rem de-randomize and extract the individual files:
rem s6_M2.txt opt957_M2.txt, rand250_M2.txt
rem the "-Ra" option also creates argyll.bat which automates
rem creating the ICC profiles.
echo i1patches -Ra P_rand250__x6__opt957_M2.txt
i1patches -Ra P_rand250__x6__opt957_M2.txt >>log.txt
echo - >>log.txt
echo on
