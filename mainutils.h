...
vector<T> change_i1_order(const vector<T>& v, int row_count, int col_count, bool to_i1Pro2)
{
    ...
    if (to_i1Pro2)
    {
        vector<vector<T>> vs(row_count * page_count, vector<T>(col_count));
        int index = 0;
        for (int page = 0; page < page_count; page++)
        {
            for (int col = 0; col < col_count; col += page_count)
                for (int row = 0; row < row_count * page_count; row++)
                    if (col + row / row_count < col_count)
                        vs[row % row_count + page * row_count][col + row / row_count] = v[index++];
        }
        for (int col = 0; col < col_count; col++)
            for (int row = 0; row < row_count * page_count; row++)
                ret.push_back(vs[row][col]);
    }
    else
    {
        vector<vector<T>> vs(row_count * page_count, vector<T>(col_count));
        ...
        for (int page = 0; page < page_count; page++)
            for (int col = 0; col < col_count; col++)
                for (int row = 0; row < row_count; row++)
                    ret.push_back(vs[row + page * row_count][col]);
